...
```
Class assigns the attributes of unique variables as factors like company.location has 61 different locations. Character is is the mode of the object that holds character strings.
...
The plots are small and could be enlarged so that the information is easier to read. The key in the bottom left corner may be obsructing some data points and should be moved. In the plot of germany the yellow points are too bright. You could include a trendline to see how the percentage of cocoa changed over time.
...
```{r}
hist(choco$Review.Date, main = "Histogram of Review Date")
```
The histogram of number of reviews is left-skewed. As the number of years increase so does the number of reviews with a peak in 2015.
...
top3 #U.S.A., France, and Canada have the most ratings

usaFrCan = choco %>%
  select(Company.Location,Rating) %>%
  filter(Company.Location %in% c('France','U.S.A.',"Canada")) %>%
  droplevels()


boxplot(usaFrCan$Rating~usaFrCan$Company.Location)
...
Is there a correlation between broad bean origin and rating?
