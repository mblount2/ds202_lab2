...
```{r}
hist(choco$Rating, main = "Histogram of Ratings", xlab = "Rating")
```

The distrubution of ratings is rougly normal with a slight left skew. There is an outlier between 4.5 and 5.


2. Do ratings depend on the cacao percentage of a chocolate bar?

```{r}
plot.default(choco$Cocoa.Pct, choco$Rating, main = "Scatterplot")
```
...
Company locations is a factor variable. This is different from a character vaiable in that it has a fixed and known set of possilbe values.
